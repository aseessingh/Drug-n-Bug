...
#Preprocessing
metabolite_data <- metabolite_data %>% mutate_at(vars(-MGS_PID), ~ log10(. + 1))
metadata_d <- metadata %>% filter(SampleID %in% dose_only_meta$SampleID) %>% select(SampleID,AGE,GENDER,CENTER_C)
metadata_ds <- metadata %>% filter(SampleID %in% dose_serum_meta$SampleID) %>% select(SampleID,AGE,GENDER,CENTER_C)
...
metabolite_d_AG <- metabolite_d %>% select(MGS_PID,'1,5-anhydroglucitol (1,5-AG)') %>% rename (AG = '1,5-anhydroglucitol (1,5-AG)')
metabolite_ds_AG <- metabolite_ds %>% select(MGS_PID,'1,5-anhydroglucitol (1,5-AG)') %>% rename (AG = '1,5-anhydroglucitol (1,5-AG)')
metadata_d <- left_join(metadata_d,metabolite_d_AG,by=c("SampleID"="MGS_PID"))
metadata_ds <- left_join(metadata_ds,metabolite_ds_AG,by=c("SampleID"="MGS_PID"))
...
metadecounfoundR_d <- perform_metadeconfound_analysis(metadata = metadata_d,metabolite_data = metabolite_d,variables = c("AG"),filtering_criteria = c("OK_nc","OK_sd"),output_number = 1,chemical_annotation = chemical_annotation)
metadeconfoundR_ds <- perform_metadeconfound_analysis(metadata = metadata_ds,metabolite_data = metabolite_ds,variables = c("AG"),filtering_criteria = c("OK_nc","OK_sd"),output_number = 2,chemical_annotation = chemical_annotation)
#Linear Regression
perform_and_extract_significant_results_AG <- function(metadata_1, metabolite_data, output_number) {
  ...
  combined_data <- combined_data %>% select(AGE, GENDER, CENTER_C, AG, everything())
  ...
  perform_linear_regression <- function(combined_data) {
    ...
    for (i in 5:ncol(combined_data)) {  # Metabolites start from column 5 to the last column
      ...
      formula <- as.formula(paste("AG ~ `", metabolite, "` + AGE + GENDER + CENTER_C", sep = ""))
  ...
  output_file <- paste0("AG_", output_number, "_lm.csv")
  write_csv(significant_results, output_file)
  
  return(significant_results)
}

lm_d <- perform_and_extract_significant_results_AG(metadata_1 = metadata_d,metabolite_data = metabolite_d,output_number = 1)
lm_ds <- perform_and_extract_significant_results_AG(metadata_1 = metadata_ds,metabolite_data = metabolite_ds,output_number = 2)
