...
metabolite_1 <- metabolites_final %>% filter(MGS_PID %in% cluster_1$SampleID)
metabolite_2 <- metabolites_final %>% filter(MGS_PID %in% cluster_2$SampleID)
rownames_function <- function(df) {
  rownames(df) <- df[, 1]
  df <- df[, -1]
  return(df)
}
metabolite_1 <- rownames_function(metabolite_1)
metabolite_2 <- rownames_function(metabolite_2)
metadata <- rbind(cluster_1,cluster_2)
metabolites <- rbind(metabolite_1,metabolite_2)
#Combined Analysis
...
results <- combined_analysis(combined_df = metabolites,alpha = 0.05,metadata_df = metadata,annotation_df=chemical_annotation,name_key = metabolites_key)
write.csv(results,"cluster_data.csv")
