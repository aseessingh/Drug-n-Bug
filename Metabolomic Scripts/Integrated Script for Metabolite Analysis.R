...
drug_dose <- c("DOSAGE_METFORMIN_C")
drug_metabolite <- c("metformin")
drug_metadata <- c("METFORMIN_C")
...
chemical_annotation <- chemical_annotation %>% select(CHEMICAL_NAME,ontology,KEGG,HMDB)
metadata <- metadata %>% select(SampleID, AGE, GENDER, CENTER_C,drug_metadata)
metadata_0 <- group_3 %>% filter(get(drug_metadata) == 1)
...
venn_df <- list(
    Group_3 = group_3$SampleID,
    Group_3_with_Metformin = metadata_0$SampleID,
    Group_3_with_Metformin_and_MGS_Data = metadata_1$SampleID,
    Group_3_with_Metformin_and_MGS_Data_and_DoseData = metadata_2$SampleID,
    Group_3_with_Metformin_and_MGS_Data_and_Dose_Serum_Data = metadata_3$SampleID
)
...
#Linear Regression
perform_and_extract_significant_results <- function(metadata_1, metabolite_data, drug_dose, drug_metabolite, output_number,chemical_annotation) {
  ...
  significant_results_dose <- left_join(significant_results_dose,chemical_annotation,by=c("metabolite"="CHEMICAL_NAME"))
  significant_results_serum <- perform_linear_regression(combined_data_serum, drug_metabolite)
  significant_results_serum <- left_join(significant_results_serum,chemical_annotation,by=c("metabolite"="CHEMICAL_NAME"))
...
lm_3 <- perform_and_extract_significant_results(metadata_1 = metadata_3,metabolite_data = metabolite_data_3,drug_dose = drug_dose,drug_metabolite = drug_metabolite,output_number = 3,chemical_annotation)
lm_4 <- perform_and_extract_significant_results(metadata_1 = metadata_4,metabolite_data = metabolite_data_4,drug_dose = drug_dose,drug_metabolite = drug_metabolite,output_number = 4,chemical_annotation)
