...
# Load data
df = pd.read_csv("combined_df_pos_neg.csv")
...
omics_ranges = {
    "Metabolomics": (18, 250),
    "KEGG_KO": (250, 1156),
    "KEGG_Module": (1156,1253),
    "KEGG_pathway" : (1253,1290),
    "GMM" : (12,18)
}
...
def stack_output_shape(input_shapes):
    return (input_shapes[0][0], len(input_shapes), input_shapes[0][1])

stacked = Lambda(stack_outputs, output_shape=stack_output_shape)(encoded_outputs)
...
model.save("outputs6/latent_model.keras")
...
for i in range(x_array.shape[1]):
    for j in range(y_array.shape[1]):
        print(f"Running SEM for x{i+1} and y{j+1}")
        x_var = x_array[:, i]
        y_var = y_array[:, j]
        ...
        with pm.Model() as sem_model:
            ...
            try:
                ...
                print(f"Sampling failed for x{i+1}, y{j+1}: {e}")
                continue
...
az_summary.to_csv("outputs6/bayesian_summary.csv")

idata = trace
waic = az.waic(idata)
loo = az.loo(idata)
with open("outputs6/model_diagnostics.txt", "w") as f:
    f.write(f"WAIC:\n{waic}\n\n")
    f.write(f"LOO:\n{loo}\n")
...
correlation_matrix.to_csv("outputs6/latent_y_correlation.csv")
pval_matrix.to_csv("outputs6/latent_y_pvalues.csv")
...
plt.tight_layout()
plt.savefig("outputs6/latent_y_correlation_heatmap.png")
plt.close()
...
partial_corr_df.to_csv("outputs6/partial_correlations.csv", index=False)

results_df = pd.DataFrame(results)
results_df.to_csv("outputs6/bayesian_sem_effects.csv", index=False)
# === 4. Save other outputs ===
np.savetxt("outputs6/latent_space.csv", latent_full, delimiter=",")
...
try:
    ...
    print("DeepExplainer failed, falling back to KernelExplainer.")
    background = omics_combined[np.random.choice(omics_combined.shape[0], 100, replace=False)]
    explainer = shap.KernelExplainer(latent_model.predict, background)
...
for i in range(len(shap_values)):
    ...
    plt.savefig(f"outputs6/shap_summary_plot_latent_dim_{i}.png")
    plt.close()


print("✅ All Bayesian SEM outputs saved in the 'outputs6' directory.")
